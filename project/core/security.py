...
from project.config import ALGORITHM, SECRET_KEY, ACCESS_TOKEN_EXPIRE_MINUTES, DOMAIN, ALGORITHMS, API_AUDIENCE, ISSUER
...
def decode_token(token: TokenResponse):
    try:
        ...
        return decoded_token.get('email') if decoded_token.get('expiry') >= time.time() else None
    except:
        return None
